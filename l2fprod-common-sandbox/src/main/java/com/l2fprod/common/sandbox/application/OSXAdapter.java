...
/**
 * A wrapper around Apple eawt classes to handle About, Preferences and Quit
 * commands. It registers a listener on the Application.
 */
public class OSXAdapter {
    ...
    public OSXAdapter() throws Exception {
        ...
        Object appListener = Proxy.newProxyInstance(Thread.currentThread().getContextClassLoader(),
                new Class[] { appListenerClass }, new Invoker());

        Method getApplication = applicationClass.getMethod("getApplication", null);
        application = getApplication.invoke(null, null);

        Method addListener = applicationClass.getMethod("addApplicationListener", new Class[] { appListenerClass });
    ...
    private void setEnabled(String name, boolean enable) throws Exception {
        Method setEnabled = applicationClass.getMethod("setEnabled" + name, new Class[] { boolean.class });
