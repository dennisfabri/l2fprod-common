 ...
 * </ul>
 * 
 * @javabean.class name="PercentLayout" shortDescription="A layout supports
 *                 constraints expressed in percent."
 */
        ...
        for (int i = 0, c = components.length; i < c; i++) {
            if (components[i].isVisible()) {
                ...
                if (constraint == null || constraint == PREFERRED_SIZE) {
                    sizes[i] = (HORIZONTAL == orientation ? components[i].getPreferredSize().width
