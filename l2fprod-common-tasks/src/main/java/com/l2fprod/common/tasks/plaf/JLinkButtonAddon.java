...
public class JLinkButtonAddon extends AbstractComponentAddon {
    ...
    protected void addBasicDefaults(LookAndFeelAddons addon, List defaults) {
        defaults.addAll(
                Arrays.asList(new Object[] { JLinkButton.UI_CLASS_ID, getPackagename() + ".basic.BasicLinkButtonUI" }));
    }

    protected void addWindowsDefaults(LookAndFeelAddons addon, List defaults) {
        defaults.addAll(Arrays
                .asList(new Object[] { JLinkButton.UI_CLASS_ID, getPackagename() + ".windows.WindowsLinkButtonUI" }));
    }
